function h = schemaball(r, lbls, ccolor, ncolor,tcolor)
...
for c = 1:N2
    ...
    if nnz(idx)
        ...
        s.l(c) = plot(Bx(:),By(:),'Color',ccolor(pos,:),'LineWidth',3);
    end
end
% PLOT NODES
...
[Z,isrt]   = sort(accumarray(subs,abs(r( row + (col-1)*sz(1) )),[],@nanmean));
% [Z,isrt]   = sort(accumarray(subs,r( row + (col-1)*sz(1) ),[],@nansum));

Z          = (Z-min(Z)+0.01)/(max(Z)-min(Z)+0.01);
ncolor     = hsv2rgb([repmat(ncolor(1:2), sz(1),1) Z*ncolor(3)]);
s.s        = scatter(x(isrt),y(isrt),[], ncolor,'fill','MarkerFaceColor',ecolor,'MarkerEdgeColor',[1 1 1],'LineWidth',1);
% PLACE TEXT LABELS such that you always read 'left to right'
ipos       = x > 0;
s.t        = zeros(sz(1),1);
s.t( ipos) = text(x( ipos)*1.1, y( ipos)*1.1, lbls( ipos),'Color',tcolor,'FontSize',20);
