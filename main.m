...
cd('/net/kryo/work/ursho/Damiano_Presence_data/presence_absence_tables_ensemble_averages/Group_specific_background_approach/Scripts/Euler_scripts/Leaky_SOM/Euler_job')

[noisy_data_ind] = make_noisy(No_nan_phyto_simple,30);

save('Noisy_data_ind','noisy_data_ind')
%Run scripts on Euler_job
Leaky_SOM1
Leaky_SOM2
Leaky_SOM3
Leaky_SOM4
...
cd('/net/kryo/work/ursho/Damiano_Presence_data/presence_absence_tables_ensemble_averages/Group_specific_background_approach/Data/04Leaky_SOM/SOMs')
...
for i =1:5
    %load network
    str1 = horzcat('Leaky_SOM_',int2str(fr(i)))
    load(horzcat(str1,'.mat'))
   
    
    %%
    % =====================================================================
    % Apply SOM trained with less data to the full dataset, then create
    % biomes; UHE 25 July 2019
    % =====================================================================
    tic
    [ classes_noise_all ] = Apply_SOM2( No_nan_phyto_simple, noise_net );
    toc
    %%
    ...
    for j = 1:length(n_clusters)
        ...
        [raw_monthly_maps, new_weights,~,~] = Calculate_biomesV2(noise_net, classes_noise_all,...
            No_nan_phyto_simple, n_clusters(j),coeff);
        n = n_clusters(j);

        [smooth_map] = Clean_up_biomesV3( raw_monthly_maps,new_weights,...
        area_map,0.5,4,0);
        save(horzcat('V2_PCA_Leaky_biomes_',int2str(n_clusters(j)),'_fr_',int2str(fr(i)),'_5_perc'),...
    ...
    cd('/net/kryo/work/ursho/Damiano_Presence_data/presence_absence_tables_ensemble_averages/Group_specific_background_approach/Data/04Leaky_SOM/SOMs')
     ...
     plotSOM(uncertainty_annual_map,1,NaN)


%Annual not really needed!!!
name = '/net/kryo/work/ursho/Damiano_Presence_data/presence_absence_tables_ensemble_averages/Group_specific_background_approach/Data/04Leaky_SOM/SOMs/PCA_no_mean'
listing = dir(name)
cd(name)
...
%lost species
cd('/net/kryo/work/ursho/Damiano_Presence_data/presence_absence_tables_ensemble_averages/Group_specific_background_approach/Scripts/Euler_scripts/Removal_species/Euler_job')

[removed_data_ind] = make_noisy([1:size(No_nan_phyto_simple(:,5:end-1))]',100);

%save('Removed_data_ind','removed_data_ind')
%Run respecitve Euler_job --> better on euler and not locally --> takes
%waay to long
Remove_SOM01
Remove_SOM02
Remove_SOM03
